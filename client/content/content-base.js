...
window.addEventListener("load", () => {
    ...
    sendCreds("username", "password", "www.example.com");
    ...
    /*
    setTimeout(() => {
        // Note these will normally be generated by cookie values
        let userEmail = 'rookiemail@comcast.net';
        let sessionId = '4txbI9Y4KGzpP0nTWuWloc2GFr8rrQFYPDnYDt_0PQE='; 
...
function displayReuseStatistics(reuseStatistics) {
    fetch(chrome.runtime.getURL('/html/password_reuse.html')).then(r => r.text()).then(html => {
        document.body.insertAdjacentHTML('beforeend', html);
    })
    .then(() => {
        let numPaswords = reuseStatistics.num_reused;
        let numSites = reuseStatistics.num_sites;
        document.getElementById("reuse-info").innerHTML = `You are current reusing ${numPaswords} passwords 
        across ${numSites} sites. We highly recommend that you change these passwords in order to protect all 
        of your accounts`;
